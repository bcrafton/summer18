...
def d(idx, vmem, vo2, iin, dvmem_dt, dvo2_dt):
    for i in range(50):
        current_idx = idx * 50 + i
        
        vmem[current_idx] = min(max(vmem[current_idx], 0.0), 1.1)    
        vo2[current_idx] = min(max(vo2[current_idx], 0.0), 1.1)

        vo1 = vo1_fit(vmem[current_idx])
            
        imem = (iin[current_idx] - m20_fit(vmem[current_idx]) + m7_fit(vmem[current_idx]) - m12_fit(vmem[current_idx], vo2[current_idx]))
        dvmem_dt[current_idx] = X1 * imem

        io2 = io2_fit(vo1, vo2[current_idx])
        dvo2_dt[current_idx] = X2 * io2
...
def deriv(t, y):
    ...
    threads = []
    for t in range(2):
        thread = threading.Thread(target=d, args=(t, vmem, vo2, iin, dvmem_dt, dvo2_dt))
...
sol = solve_ivp(deriv, (1e-4, 4e-2), y0, method='RK45')
