...
#         connListSparse = ([(i,j[0],j[1]) for i in xrange(connMatrix.shape[0]) for j in zip(connMatrix.rowj[i],connMatrix.rowdata[i])])
        ...
        np.save('../saved_weights/' + connName + ending, connListSparse)

def save_theta(ending = ''):
    print 'save theta'
    for pop_name in population_names:
        np.save('../saved_weights/theta_' + pop_name + ending, neuron_groups[pop_name + 'e'].theta)
...
while j < (int(num_examples)):
    ...
    rates = training['x'][j % 60000, :, :].reshape(n_input) * 32. *  input_intensity
