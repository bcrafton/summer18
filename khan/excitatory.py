...
while ex < NUM_EX:
    ...
    for s in range(active_steps):
        ...
        if args.train:
            label = training_labels[ex]
            labels[ex] = label
            input_intensity = np.average(intensities[label]) + inc
            rates = training_set[ex] * 32.0 * input_intensity
        else:
            label = testing_labels[ex]
            labels[ex] = label
            input_intensity = np.average(intensities[label]) + inc
    ...
    print "----------"
    print ex, label, input_intensity, dt
    ...
    if np.sum(spk_count[ex]) < 5:
        spk_count[ex] = 0
        inc += 0.1
    elif np.sum(spk_count[ex]) > 100:
        spk_count[ex] = 0
        Syn.w = prev_weights
        dt = 0.5 * dt
        # skip it.
        if dt < 1e-6 or input_intensity > 3.0:
            inc = 0.0
            dt = 0.5e-3
            ex += 1
    else:
        intensities[label].append(input_intensity)
        inc = 0.0
        dt = 0.5e-3
        ex += 1
