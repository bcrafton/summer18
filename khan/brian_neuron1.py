...
neuron_groups['e'].theta = np.load('theta_A.npy') * b2.volt
...
state_mon = b2.StateMonitor(neuron_groups['Ae'], ('v'), True)
...
while j < num_examples:
    print j, np.sum(spike_counters['Ae'].count[:]), input_intensity
...
'''
t = np.linspace(0, 5.0, len(state_mon.v[0]))
plt.plot(t, state_mon.v[0])
plt.savefig('plt.png')
'''

print np.sum(current_spike_count)
print spike_counters['Ae'].count[:]

# print len(state_mon.v[0])

t = np.linspace(0, 5.0, len(state_mon.v[0]))
plt.plot(t, np.transpose(state_mon.v[2]))
# plt.plot(t, np.transpose(state_mon.v[0:50]))
# plt.plot(t, state_mon.v[0], t, state_mon.v[1], t, state_mon.v[2])
plt.savefig('plt.png')
