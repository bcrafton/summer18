...
runtimes = open("runtimes_test", "w")
runtimes.write("runtimes testing\n")
runtimes.close()
...
for ex in range(NUM_EX):
    ...
    if (ex % 10 == 0):
        ...
        runtimes = open("runtimes_test", "a")
        runtimes.write("Example # " + str(ex) + " / " + str(NUM_EX) + " | " + str(time.time() - start) + "\n")
        runtimes.close()
    
    if (ex % 100 == 0):
        np.save('./results/test_spks_'   + str(ex), spk_count[0:ex])
        np.save('./results/test_labels_' + str(ex), labels[0:ex])
    ...
    while spks < 5:
        ...
        for s in range(steps):
            ...
            rates = testing_set[ex] * 32.0 * input_factor
...
np.save('./results/test_spks_'   + str(NUM_EX), spk_count)
np.save('./results/test_labels_' + str(NUM_EX), labels)
