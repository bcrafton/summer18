...
b = tf.placeholder(tf.float32, [101, 10])
##############################################
...
B = np.random.uniform(0.25, 0.75, size=(101, 10)) * (2 * 0.12) - 0.12
for ii in range(101):
    B[ii] = B[ii] * one_hot(np.random.randint(0, 9), 9)

# TODO dont want to be sending B every single batch. figure out how to stick it in a variable or some shit.

for ii in range(500 * 1000 * 10):
    batch_xs, batch_ys = mnist.train.next_batch(100)
    sess.run([W1,W2], feed_dict={X: batch_xs, b: B, ANS: batch_ys})
    print(ii)

correct_prediction = tf.equal(tf.argmax(A3,1), tf.argmax(ANS,1))
accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
print(sess.run(accuracy, feed_dict={X: mnist.test.images, b: B, ANS: mnist.test.labels}))
