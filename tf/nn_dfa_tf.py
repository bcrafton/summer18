...
class nn_dfa_tf:
    def __init__(self, size, weights, B, alpha, bias):
    ...
    def train(self, x, y):
        ...
        for ii in range(self.num_layers-1, 0, -1):
            ...
            if ii == self.num_layers-1:
                ...
                SigGrad = tf.multiply(tf.sigmoid(Z[ii]), tf.subtract(1.0, tf.sigmoid(Z[ii])))
                D[ii] = tf.multiply(tf.matmul(D[self.num_layers-1], tf.transpose(self.B[ii])), SigGrad)
