...
W2 = tf.Variable(tf.random_uniform(shape=[5, 5, 128, 256]) * (2 * 0.12) - 0.12)
l2 = Convolution(input_sizes=[batch_size, 32, 32, 128], filter_sizes=[5, 5, 128, 256], filters=W2, stride=1, padding=1, alpha=ALPHA, activation=Relu(), last_layer=False)

l3 = MaxPool(size=[batch_size, 32, 32, 256], stride=[1, 2, 2, 1])

l4 = ConvToFullyConnected(shape=[16, 16, 256])

W5 = tf.Variable(tf.random_uniform(shape=[16*16*256, 1024]) * (2 * 0.12) - 0.12)
l5 = FullyConnected(size=[16*16*256, 1024], weights=W5, alpha=ALPHA, activation=Relu(), last_layer=False)

W6 = tf.Variable(tf.random_uniform(shape=[1024, 10]) * (2 * 0.12) - 0.12)
l6 = FullyConnected(size=[1024, 10], weights=W6, alpha=ALPHA, activation=Relu(), last_layer=False)
