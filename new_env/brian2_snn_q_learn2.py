...
class Solver():
    def __init__(self, n_episodes=1000, max_env_steps=None, gamma=1.0, epsilon=0.5, epsilon_min=0.01, epsilon_decay=0.99, alpha=0.10, alpha_decay=0.10, batch_size=1, quiet=False):
        ...
        self.weights = np.random.normal(10.0, 0.0, size=(16,4))
        ...
        while (done == False):
          ...
          if start:
              ...
              if (np.random.random() <= self.epsilon):
                  ...
                  # action = np.random.choice(np.flatnonzero(current_spike_count == current_spike_count.max()))

              print (current_spike_count)

          next_state, reward, done = self.env.step(action)
          next_state = num_to_state(next_state)
          print (state_to_num(state), action, state_to_num(next_state))
          ...
          gradient = elig * reward * (1/500)
          gradient = np.array(gradient)
          self.weights = self.weights + gradient

          mat = self.weights * ((w_max-w_min) * 10 / np.absolute(np.average(self.weights)))
          mat = mat + np.absolute(np.min(mat))
          io_syn.w = mat.reshape(64)
