...
class Solver():
    def __init__(self, n_episodes=1000, max_env_steps=None, gamma=1.0, epsilon=0.5, epsilon_min=0.01, epsilon_decay=0.99, alpha=0.04, alpha_decay=0.04, batch_size=32, quiet=False):
        ...
        weights0 = np.load("snn_weights.npy")
        weights0 = weights0.reshape(16,4)

        weights1 = np.ones(4)

        pass_weights = (weights0, weights1)

        '''
        weight1 = self.model.get_weights()[0]
        weight1 = np.asarray(weight1)
        print (np.shape(weight1))

        weight1 = self.model.get_weights()[1]
        weight1 = np.asarray(weight1)
        print (np.shape(weight1))
        '''
        
        self.model.set_weights(pass_weights)
