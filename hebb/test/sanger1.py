...
for itr in range(args.iters):
  for i in range(args.examples):
    ...
    wy = np.dot(w, y)
    d = x - wy
    w = w + args.lr * d.reshape(784, 1) * y.reshape(1, 400)
...
for i in range(args.examples):
  ...
  for j in range(layer2):
    if max_score[j] < percentileofscore(y, y[j]):
      max_score[j] = percentileofscore(y, y[j])
