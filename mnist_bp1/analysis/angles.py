...
DFA_D2 = tf.multiply(tf.matmul(D3, tf.transpose(B)), dsigmoid(A2))
BP_D2 = tf.multiply(tf.matmul(D3, tf.transpose(W2)), dsigmoid(A2))
D2 = DFA_D2
...
last_50_dw_angles = deque(maxlen=50)
dw_angles = []

last_50_losses = deque(maxlen=50)
losses = []
...
for ii in range(EPOCHS):
    for jj in range(int(TRAIN_EXAMPLES / BATCH_SIZE)):
        ...
        w1, w2, l, dfa_d2, bp_d2 = sess.run([W1, W2, loss, DFA_D2, BP_D2], feed_dict={ALPHA: 0.01, X: xs, ANS: ys})
        ...
        #losses.append(l)
        last_50_losses.append(l)
        losses.append(np.average(last_50_losses))
...
plt.subplot(412)
plt.plot(dw_angles)
plt.xlabel("Angle dW2 DFA vs dW2 BP")
