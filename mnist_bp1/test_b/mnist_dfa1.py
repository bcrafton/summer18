...
for ii in range(LAYER2):
    ...
    b2[ii][idx] = np.random.normal(0.0, sqrt_fan_out*0.01)
'''
...
a = np.random.uniform(-sqrt_fan_out, sqrt_fan_out, size=(LAYER2 + 1, LAYER3))

print np.average(b2), np.std(b2)
print np.average(a), np.std(a)

print np.linalg.matrix_rank(b2, tol=1e-3)
print np.linalg.matrix_rank(a, tol=1e-3)
