...
b2 = np.random.uniform(0.0, 1.0, size=(LAYER2 + 1, LAYER4)) * 2 * EPSILON - EPSILON
b3 = np.random.uniform(0.0, 1.0, size=(LAYER3 + 1, LAYER4)) * 2 * EPSILON - EPSILON
...
for epoch in range(NUM_TRAIN_EPOCHS):    
    ...
    for ex in range(NUM_TRAIN_EXAMPLES):
        ...
        D4 = A4 - ANS
        D3 = np.dot(D4, np.transpose(b3)) * np.append(sigmoid_gradient(Z3), 1)
        D2 = np.dot(D4, np.transpose(b2)) * np.append(sigmoid_gradient(Z2), 1)
