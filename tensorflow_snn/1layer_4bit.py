...
def num_to_state(state):
    state = '{0:04b}'.format(state)
    ret = np.zeros(4)
    for i in range(4):
        if state[i] == '1':
            ret[i] = 1
    return np.reshape(ret, [1, 4])
...
class Solver():
    ...
    def run(self):
        ...
        #############
        Wsyn = np.random.normal(1.0, 0.25, size=(4,4))
        ...
        for e in range(5000):
            ...
            while not done:
                ...
                input_fires = np.zeros(shape=(2000,4))
                input_fired_counts = np.zeros(4)
                ...
                for t in range(time_steps):
                    input_fired = np.random.rand(1, 4) < rates * dt
                ...
                post = np.zeros(shape=(4,4))
                pre = np.zeros(shape=(4,4))
                for i in range(4):
                    for j in range(4):
                ...
                elig[state_to_num(state)][action] = 4
                elig = elig - 1
